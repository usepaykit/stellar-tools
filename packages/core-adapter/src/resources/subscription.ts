...
export class SubscriptionApi {
  ...
  async create(params: CreateSubscription): Promise<Result<Subscription, Error>> {
    ...
    if (!response.ok) {
      return ERR(new Error(`Failed to create subscription: ${response.error?.message}`));
    }
  ...
  async retrieve(id: string): Promise<Result<Subscription, Error>> {
    ...
    if (!response.ok) {
      return ERR(new Error(`Failed to retrieve subscription: ${response.error?.message}`));
    }
  ...
  async list(customerId: string): Promise<Result<Subscription[], Error>> {
    ...
    if (!response.ok) {
      return ERR(new Error(`Failed to list subscriptions: ${response.error?.message}`));
    }
  ...
  async pause(id: string): Promise<Result<Subscription, Error>> {
    ...
    if (!response.ok) {
      return ERR(new Error(`Failed to pause subscription: ${response.error?.message}`));
    }
  ...
  async resume(id: string): Promise<Result<Subscription, Error>> {
    ...
    if (!response.ok) {
      return ERR(new Error(`Failed to resume subscription: ${response.error?.message}`));
    }

    return OK(response.value.data);
  }

  async update(id: string, params: UpdateSubscription): Promise<Result<Subscription, Error>> {
    ...
    const response = await this.apiClient.put<Subscription>(`/subscriptions/${id}`, {
      body: JSON.stringify(data),
    });

    if (!response.ok) {
      return ERR(new Error(`Failed to update subscription: ${response.error?.message}`));
    }
  ...
  async cancel(id: string): Promise<Result<Subscription, Error>> {
    ...
    if (!response.ok) {
      return ERR(new Error(`Failed to cancel subscription: ${response.error?.message}`));
    }
