...
import { Webhook, type WebhookEvent as WebhookEventType, webhookEvent } from "@stellartools/core";
...
const getCurlExample = (event: string) => /* sh */ `curl -X POST https://your-domain.com/api/webhooks \\
...
# Available events: ${webhookEvent.map((e) => e).join(", ")}
`;
...
const WEBHOOK_EVENTS = webhookEvent.map((id) => ({
  id,
  label: formatEventLabel(id),
})) as { id: WebhookEventType; label: string }[];
            ...
            <Tabs defaultValue="ts">
              ...
              <TabsContent value="ts" className="pt-2">
                <CodeBlock language="typescript" filename="api/webhook/route.ts" maxHeight="400px">
                  {webhookSecret ? getTsExample(webhookSecret) : WEBHOOK_TYPESCRIPT_EXAMPLE}
                </CodeBlock>
              </TabsContent>
