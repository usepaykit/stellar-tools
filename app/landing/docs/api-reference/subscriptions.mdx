...
**Response** `200`: `{ "data": { "count": 2, "failed": [] } }` — `failed` lists any customers that could not be subscribed (e.g. missing wallet).
...
Pauses the subscription on-chain. Requires `customerAddress` (Stellar public key) and `productId`.
...
Resumes a paused subscription. Body must include `customerAddress` and `productId`.
...
Cancels the subscription on-chain and updates status. Body must include `customerAddress` and `productId`.
